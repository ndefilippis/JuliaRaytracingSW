...
function initialize_problem(Npackets, umax)
    ...
    common_params = (; Lx, nx, dt, T=Float32, nν, ν, order, use_filter, aliased_fraction=1/3)
    ...
    println(@sprintf("Total steps: %d. Spin-up steps: %d, Output every %d steps. Total: %d output frames. Diagnostics every %d steps, max writes per file: %d", nsteps, spinup_step, output_freq, (nsteps - spinup_step) / output_freq, diags_freq, Parameters.max_writes))
