...
function simulate!(nsteps, nsubs, npacketsubs, grid, prob, packets, base_filename, packetSpinUpDelay, packet_params)
    ...
    u_background  = Array{Float64}(undef, grid.nx, grid.ny)
    v_background  = Array{Float64}(undef, grid.nx, grid.ny)
    ux_background = Array{Float64}(undef, grid.nx, grid.ny)
    uy_background = Array{Float64}(undef, grid.nx, grid.ny)
    vx_background = Array{Float64}(undef, grid.nx, grid.ny)
    vy_background = Array{Float64}(undef, grid.nx, grid.ny)
    
    velocity = Raytracing.Velocity(u_background, v_background)
    grad_v = Raytracing.VelocityGradient(ux_background, uy_background, vx_background, vy_background)
    ...
    get_velocity_info((@views(prob.vars.ψh[:,:,1]) + @views(prob.vars.ψh[:,:,2]))/2, grid, packet_params, velocity, grad_v, temp_device_in_field, temp_device_out_field);
    ...
    for j=frames
        ...
        old_t = clock.t
        new_t = old_t + packet_params.dt * npacketsubs
        Raytracing.solve!(velocity, grad_v, grid.x, grid.y, packet_params.Npackets, packets, packet_params.dt, (packet_params.packetVelocityScale * old_t, packet_params.packetVelocityScale * new_t), packet_params);
