...
function set_up_grid(directory)
    setup_file = jldopen(@sprintf("%s/2Lqg.%06d.jld2", directory, 0), "r")
...
function get_baroclinic_streamfunction(sol, grid, params)
    qch = @views sol[:,:,1] - sol[:,:,2]
...
function get_total_frames(directory, file_indices)
    ...
    for i=file_list
        file = jldopen(@sprintf("%s/2Lqg.%06d.jld2", directory, i))
        println(file)
...
function write_fourier_data(directory, file_indices, k_idx)
    ...
    for i=file_indices
        file = jldopen(@sprintf("%s/2Lqg.%06d.jld2", directory, i), "r")
        frames = keys(file["snapshots/t"])
        for frame_idx=1:length(frames)
            ...
            ψh = get_baroclinic_streamfunction(qh, grid, params)
    ...
    output_file["t"] = t
    output_file["ψt"]  = clean_fft(t, ψt, window)
    close(output_file)
