...
function Vars(grid)
  ...
  @devzeros Dev T (grid.nx, grid.ny) u v m η ζ
  @devzeros Dev Complex{T} (grid.nkr, grid.nl) uh vh mh ηh ζh 

  return Vars(u, v, m, η, ζ, uh, vh, mh, ηh, ζh)
end
...
function updatevars!(prob)
    ...
    @. vars.vh = @view sol[:,:,2]
    @. vars.mh = @view sol[:,:,3]
    @. vars.ζh = 1im * grid.kr * vars.vh - 1im * grid.l * vars.uh
    ...
    ldiv!(vars.m, grid.rfftplan, deepcopy(vars.mh)) # use deepcopy() because irfft destroys its input
...
function enforce_reality_condition!(prob)
    ...
    mul!(vars.mh, grid.rfftplan, deepcopy(vars.m))
...
function calcN!(N, sol, t, clock, vars, params, grid)
    ...
    #===
    Compute modified nonlinear pressure term, using m=1/(1+η), and F(η) = 3/2-1/(2(1+η)^2)
    ===#

    mh = vars.ζh
    @. mh = vars.mh
    ldiv!(vars.m, grid.rfftplan, mh)
    ...
    muxh = vars.ζh
    @. mux = vars.u .* vars.m
    mul!(muxh, grid.rfftplan, mux)
    @. mhN = -1im * grid.kr * muxh

    mvy  = vars.ζ
    mvyh = vars.ζh
    @. mvy = vars.v .* vars.m
    mul!(mvyh, grid.rfftplan, mvy)
    @. mhN += -1im * grid.l * mvyh
...
function Lop_kernel(result, k, l, Nx, Ny, D, f, Cg2)
    ...
    result[i, j, 1, 2] =  f
    result[i, j, 1, 3] = 0 # New term is no longer linear in m

    result[i, j, 2, 1] = -f
    result[i, j, 2, 2] = D[i,j]
    result[i, j, 2, 3] = 0 # New term is no longer linear in m

    result[i, j, 3, 1] = 0 # No longer linear in m
    result[i, j, 3, 2] = 0 # No longer linear in m
...
function potential_energy(mh, params, grid)
    CUDA.@allowscalar return 0.5 * params.Cg2 * real(mh[1,1]) / (grid.Lx * grid.Ly)
...
@inline potential_energy(sol, vars, params, grid) = potential_energy(vars.mh, params, grid)
