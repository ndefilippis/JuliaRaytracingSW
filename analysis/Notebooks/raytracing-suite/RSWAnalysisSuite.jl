...
function start!(run_directory, tag; force_recompute=false)
    ...
    fig_opts = (; fontsize=32, fonts = (; regular = "Dejavu"))
...
function plot_data(run_directory, data_file, grid, tag, fig_opts)
    ...
    plot_snapshots(run_directory, data_file, grid, tag, (; size=(600, 500), fig_opts))
end
...
function plot_snapshot(time, snapshot, grid, params, title_tag, tag, fig_opts)
    ...
    q_hm = heatmap!(q_ax, grid.x, grid.y, q; colorrange=(-q_max, q_max), colormap=:balance)
    div_hm = heatmap!(div_ax, grid.x, grid.y, div; colorrange=(-div_max, div_max), colormap=:delta)
...
function plot_radial_energetics(run_directory, data_file, grid, tag, fig_opts)
    ...
    Legend(ave_ke_energy_fig[2, 1], ave_ke_energy_ax, orientation=:horizontal)
    Legend(ic_ke_energy_fig[2, 1],  ic_ke_energy_ax,  orientation=:horizontal)

    Legend(ave_pe_energy_fig[2, 1], ave_pe_energy_ax, orientation=:horizontal)
    Legend(ic_pe_energy_fig[2, 1],  ic_pe_energy_ax,  orientation=:horizontal)

    Legend(ave_e_energy_fig[2, 1], ave_e_energy_ax, orientation=:horizontal)
    Legend(ic_e_energy_fig[2, 1],  ic_e_energy_ax,  orientation=:horizontal)
